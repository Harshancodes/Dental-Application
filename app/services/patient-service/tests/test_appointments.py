...
def sample_data(client, auth_headers):
    ...
    patient = client.post("/patients/", json={"name": "Test Patient"}, headers=auth_headers).json()
    doctor = client.post("/doctors/", json={"name": "Test Doctor", "specialization": "General"}, headers=auth_headers).json()
    return {"patient_id": patient["id"], "doctor_id": doctor["id"]}


def test_create_appointment(client, auth_headers, sample_data):
...
def test_create_appointment_invalid_patient(client, auth_headers, sample_data):
...
def test_filter_appointments_by_patient(client, auth_headers, sample_data):
    ...
    resp = client.get(f"/appointments/?patient_id={sample_data['patient_id']}", headers=auth_headers)
    assert resp.status_code == 200
    assert all(a["patient_id"] == sample_data["patient_id"] for a in resp.json())


def test_cancel_appointment(client, auth_headers, sample_data):
    ...
    resp = client.patch(f"/appointments/{appt['id']}/cancel", headers=auth_headers)
    assert resp.status_code == 200
    assert resp.json()["status"] == "cancelled"


def test_delete_appointment(client, auth_headers, sample_data):
    ...
    resp = client.delete(f"/appointments/{appt['id']}", headers=auth_headers)
